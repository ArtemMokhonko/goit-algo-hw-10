...
def plot_integral(f, a, b, x_random, y_random, under_curve):
    ...
    ax.scatter(x_random, y_random, color='blue', s=1, label='Випадкові точки')
    ax.scatter(x_random[under_curve], y_random[under_curve], color='yellow', s=1, label='Точки під кривою')
...
def display_results(integral_mc, integral_analytical, error, N):
    results = f"{'Метод':<15}{'Результат':<25}{'Похибка':<25}\n"
    results += "="*65 + "\n"
